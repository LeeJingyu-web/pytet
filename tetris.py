...
class Tetris():
    ...
    def deleteFullLines(self): # To be implemented!!
        getdy = self.currBlk.get_dy()
        setzero = 0

        if self.top + self.currBlk.get_dy() >= self.iScreenDy:
            getdy = self.iScreenDy - self.top
        array = self.createArrayScreen()
        screen = Matrix(array)

        zero = screen.clip(0, 0, 1, screen.get_dx())
        for i in range(getdy - 1, -1, -1):
            cycle = self.top + i + setzero
            line = self.oScreen.clip(cycle, 0, cycle + 1, self.oScreen.get_dx())
            if line.sum() == self.oScreen.get_dx():
                temp = self.oScreen.clip(0, 0, cycle, self.oScreen.get_dx())
